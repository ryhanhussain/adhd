...
import { callGemini, checkAndIncrementQuota, getLocalDateFromRequest, getUserFromRequest } from "../_shared";
...
export async function POST(req: NextRequest) {
  ...
  const prompt = `Parse this brain dump into future tasks only. Split distinct tasks even if punctuation is messy.

Rules:
- Return only tasks the user needs or intends to do in the future.
- Ignore completed/past activity logs, reflections, journaling, habits, categories, life areas, moods, and vague fragments.
...
Transcript: ${JSON.stringify(text.trim())}`;
  ...
  try {
    const responseText = await callGemini(prompt, { temperature: 0.15, maxOutputTokens: 640 });
